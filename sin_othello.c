...
void Board_Decide() {
    ...
    for (int i = 0; i < DIR; i++) {
        ...
        if (!OutRange(tg) && field[tg.x][tg.y] == enemyNum) {
...
void NodeToArray(Node* head) {
    ...
    else
    {
        if (IsBoardFull()) {
            ...
            const char* enemyName = (step_mode == CPU) ? "CPU" : "PlayerB";
            const char* winner;
            if (pcount > ecount)
                winner = "○-PlayerA";
            else if (pcount < ecount)
                winner = (step_mode == CPU) ? "●-CPU" : "●-PlayerB";
            else
                winner = "引き分け";

            printf("○ - :%d　● - :%d 勝者: %s\n", pcount, ecount, winner);
